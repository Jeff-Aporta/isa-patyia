/**
 * QA: Guardar en langlab (Neon vía orquestador) + lectura post-guardado (simula F5).
 * Uso: node scripts/qa-save-f5.mjs
 */
const ORCH = (process.env.ORCH_URL || "https://main-orchestrator.jeffaporta.workers.dev").replace(/\/$/, "");
...
async function jfetch(path, init = {}) {
  const r = await fetch(`${ORCH}/api${path}`, init);
...
async function readPg() {
  ...
  return { status: r.status, ok: data.ok, body: row?.INSTRUCCION || "", error: data.error };
}
...
(async () => {
  console.log("=== QA save + F5 read (orquestador CF) ===\n");
  console.log("ORCH:", ORCH);

  const before = await readPg();
  report("1. GET /patyia/instrucciones", before.ok, `status=${before.status} len=${before.body.length} ${before.error || ""}`);

  const session = await login();
  report("2. Login", true, USER);

  const svc = await serviceToken(session);
  report("3. service-token langlab.guardar", Boolean(svc), "ok");

  const patched = `${before.body}\n\n${MARKER}`;
  await save(patched, svc);
  report("4. POST upsert-sql (langlab)", true, `marker=${MARKER}`);
