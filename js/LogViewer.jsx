...
const { mdToHtml, MetaBadges, MetaDialog } = PatyShared;

const { ButtonIconify } = PatyIconify;

const { JsonCodeEditor } = PatyJsonEditor;
...
function LogViewer({ bootLog = {} }) {
  ...
  const aplicarLog = useCallback((log, { silent = false } = {}) => {
    ...
    const vista = PatyConvLog.logToMensajesVista(log);
    ...
    if (!vista.length) {
      ...
      setError(msg);
      if (!silent) PatyNotify.toastWarning(msg);
    } else {
      setError("");
      if (!silent) PatyNotify.toastSuccess(`${vista.length} mensaje(s) cargado(s)`);
    }
  ...
  useEffect(() => {
    ...
    try {
      ...
      aplicarLog(PatyConvLog.parseLogInput(bootLog.jsonInput), { silent: true });
  ...
  useEffect(() => {
    ...
    PatyUrlState.mergePartial({ log: { convId, jsonInput } });
  ...
  const parsearPegado = useCallback(() => {
    ...
    try {
      ...
      const log = PatyConvLog.parseLogInput(jsonInput);
  ...
  const recuperarPorId = useCallback(async () => {
    ...
    try {
      ...
      const log = await PatyLabApi.fetchConvLogById(convId);
...
window.PatyLogViewer = { LogViewer };
